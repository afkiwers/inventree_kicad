...
class KiCadLibraryPlugin(
    UrlsMixin, AppMixin, SettingsMixin, SettingsContentMixin, InvenTreePlugin
):
    ...
    DESCRIPTION = _(
        "Provides external library functionality for KiCad via the HTTP library interface"
    )
    ...
    SETTINGS = {
        ...
        "KICAD_ENABLE_STOCK_COUNT_FORMAT": {
            ...
            "description": _(
                "This will be displayed after the part's description in KiCad (right column). Note: {1} contains the Stock information, {0} the description of the part."
            ),
            "default": "[Stock: {1}] {0}",
        },
        "DEFAULT_FOR_MISSING_SYMBOL": {
            ...
            "description": _(
                "This backup symbol will be used if none has been defined"
            ),
            "default": "",
        },
        "KICAD_INCLUDE_IPN": {
            "name": _("Include IPN"),
            "description": _(
                "When activated, the IPN is included in the KiCad fields for a part. Note: Including IPN as value will supercede the KICAD_VALUE_PARAMETER setting."
            ),
            "choices": [
        ...
        "KICAD_FIELD_VISIBILITY_PARAMETER": {
            ...
            "description": _(
                "Set field visibility in KiCad using this parameter. Enter comma-separated InvenTree parameter names to show per part."
            ),
            "model": "common.parametertemplate",
        },
        "KICAD_FIELD_VISIBILITY_PARAMETER_GLOBAL": {
            ...
            "description": _(
                "Global version of KICAD_FIELD_VISIBILITY_PARAMETER; overridden if set locally."
            ),
        ...
        "KICAD_EXCLUDE_FROM_BOM_PARAMETER": {
            ...
            "description": _(
                "The part parameter to use for to exclude it from the BOM."
            ),
            "model": "common.parametertemplate",
        },
        ...
        "KICAD_EXCLUDE_FROM_SIM_PARAMETER": {
            ...
            "description": _(
                "The part parameter to use for to exclude it from the simulation."
            ),
            "model": "common.parametertemplate",
        },
        ...
        "IMPORT_INVENTREE_ID_IDENTIFIER": {
            ...
            "description": _(
                "This identifier specifies what key the import tool looks for to get the part ID"
            ),
            "default": "InvenTree",
        },
        "KICAD_ENABLE_MANUFACTURER_DATA": {
            ...
            "description": _(
                "When activated, the supplier and manufacturer data will be added to the KiCad parts."
            ),
            "validator": bool,
            "default": False,
        },
        "KICAD_INCLUDE_UNITS_IN_PARAMETERS": {
            ...
            "description": _(
                "When activated, if a parameter has units it will be included"
            ),
            "validator": bool,
            "default": True,
        },
        "KICAD_HIDE_INACTIVE_PARTS": {
            ...
            "description": _(
                "When activated, inactive parts will be hidden from the KiCad parts preview list"
            ),
            "validator": bool,
            "default": True,
        },
        "KICAD_HIDE_TEMPLATE_PARTS": {
            ...
            "description": _(
                "When activated, template parts will be hidden from the KiCad parts preview list"
            ),
            "validator": bool,
            "default": False,
        },
    ...
    def get_settings_content(self, request):
        ...
        try:
            ...
            return render_to_string(
                "inventree_kicad/kicad_bom_import.html",
                context={
    ...
    def setup_urls(self):
        ...
        api_category_router.register(
            r"category", viewsets.CategoryApi, basename="selectedcategory"
        )
        ...
        return [
                ...
                include([
                    re_path(
                        ...
                        include([
                            re_path(
                                "category/(?P<id>.+).json$",
                                viewsets.PartsPreviewList.as_view(),
                                {"plugin": self},
                                name="kicad-part-category-list",
                            ),
                            re_path(
                                "(?P<pk>.+).json$",
                                viewsets.PartDetail.as_view(),
                                {"plugin": self},
            ...
            re_path(
                r"upload(?:\.(?P<format>json))?$",
                self.import_meta_data,
                name="meta_data_upload",
            ),
            re_path(
                r"progress_bar_status",
    ...
    def get_import_progress(self, request):
        ...
        return JsonResponse(
            {"value": progress.current_progress, "file_name": progress.file_name},
            status=200,
        )
    ...
    def import_meta_data(self, request):
        ...
        if request.FILES.get("file", False):
            ...
            kicad_footprint_param_id = self.get_setting(
                "KICAD_FOOTPRINT_PARAMETER", None
            )
            kicad_reference_param_id = self.get_setting(
                "KICAD_REFERENCE_PARAMETER", None
            )
            kicad_symbol_param_id = self.get_setting("KICAD_SYMBOL_PARAMETER", None)

            if (
                kicad_footprint_param_id == ""
                or kicad_reference_param_id == ""
                or kicad_symbol_param_id == ""
            ):
                return JsonResponse(
                    {
            ...
            import_progress = ProgressIndicator.objects.get_or_create(
                user=request.user
            )[0]
            ...
            for idx, comp in enumerate(components.findall("comp")):
                ...
                inventree_id_identifier = self.get_setting(
                    "IMPORT_INVENTREE_ID_IDENTIFIER", None
                )

                for field in fields:
                    if (
                        str(field.attrib.get("name", ""))
                        .lower()
                        .startswith(inventree_id_identifier.lower())
                    ):
                ...
                try:
                    ...
                    if invalid_part:
                        logger.debug(
                            f"Part ID: {inventree_part_id} does not belong to an existing part, skipping"
                        )
                        continue

                except Exception as exp:
                    logger.debug(
                        f"Part ID: {inventree_part_id} caused uknown error {exp}"
                    )
                    continue
                ...
                for idx, t_id in enumerate(t_ids):
                    ...
                    # Don't override
                    if created or self.get_setting(
                        "IMPORT_INVENTREE_OVERRIDE_PARAS", None
                    ):
                        parameter.data = t_id_values[idx]
                        parameter.save()

                if datasheet and str2bool(
                    self.get_setting("KICAD_META_DATA_IMPORT_ADD_DATASHEET", False)
                ):
                    try:
    ...
    def add_attachment(self, part_id, link):
        ...
        try:
            ...
            attachment = Attachment.objects.filter(
                model_type="part", model_id=part_id, comment__iexact="datasheet"
            )

            if not attachment.exists():
                Attachment.objects.create(
                    model_type="part", model_id=part_id, link=link, comment="Datasheet"
                )
        ...
        try:
            ...
            attachment = PartAttachment.objects.filter(
                part=part_id, comment__iexact="datasheet"
            )

            if not attachment.exists():
                PartAttachment.objects.create(
                    part=part_id, link=link, comment="Datasheet"
                )
